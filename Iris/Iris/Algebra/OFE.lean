...
/-- Complete ordered family of equivalences -/
class abbrev COFE {SI : outParam <| Type _} [SIdx SI] (α : Type _) := OFE α, IsCOFE α
...
instance {SI} [SIdx SI] : COFE (DiscreteO α) := COFE.ofDiscrete _

instance {SI} [SIdx SI] {α : Type _} : OFE.Discrete (DiscreteO α) := ⟨fun h => h⟩
...
theorem DiscreteO.dist_inj [SIdx SI] {x y : α} {n} (H : DiscreteO.mk x ≡{n}≡ DiscreteO.mk y) : x = y :=
