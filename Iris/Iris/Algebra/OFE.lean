...
@[rocq_alias ofe]
class OFE [instSI : SIdx] (α : Type _) where
  Dist : instSI.I → α → α → Prop
...
variable [instSI : SIdx]

@[rocq_alias dist_equivalence]
theorem dist_equivalence [OFE α] {n : instSI.I} : Equivalence (Dist (α := α) n) := dist_eqv

@[rocq_alias dist_lt]
theorem Dist.lt [OFE α] {m n : instSI.I} {x y : α} : x ≡{n}≡ y → m < n → x ≡{m}≡ y := dist_lt

/-

@[rocq_alias dist_le]
theorem Dist.le [OFE α] {m n : instSI.I} {x y : α} (h : x ≡{n}≡ y) (h' : m ≤ n) : x ≡{m}≡ y :=
...
@[simp, refl] theorem Dist.rfl [OFE α] {n : instSI.I} {x : α} : x ≡{n}≡ x := dist_eqv.1 _
@[symm] theorem Dist.symm [OFE α] {n : instSI.I} {x : α} : x ≡{n}≡ y → y ≡{n}≡ x := dist_eqv.2
theorem Dist.trans [OFE α] {n : instSI.I} {x : α} : x ≡{n}≡ y → y ≡{n}≡ z → x ≡{n}≡ z := dist_eqv.3
...
instance [OFE α] {n : instSI.I} : Trans (OFE.Dist n) (OFE.Dist n) (OFE.Dist n : α → α → Prop) where
  trans := Dist.trans
...
def DistLater [OFE α] (n : instSI.I) (x y : α) : Prop := ∀ m, m < n → x ≡{m}≡ y

@[simp, refl] theorem DistLater.rfl [OFE α] {n : instSI.I} {x : α} : DistLater n x x := fun _ _ => .rfl
@[symm] theorem DistLater.symm [OFE α] {n : instSI.I} {x : α} (h : DistLater n x y) : DistLater n y x :=
  fun _ hm => (h _ hm).symm
theorem DistLater.trans [OFE α] {n : instSI.I} {x : α} (h1 : DistLater n x y) (h2 : DistLater n y z) :
...
theorem distLater_eqv [OFE α] {n : instSI.I} : Equivalence (α := α) (DistLater n) where
...
theorem Dist.distLater [OFE α] {n : instSI.I} {x y : α} (h : x ≡{n}≡ y) : DistLater n x y :=
  fun _ => dist_lt h

/-- `DistLater n`-equivalence implies `m`-equivalence for all `m < n`. -/
@[rocq_alias dist_later_dist_lt]
theorem DistLater.dist_lt [OFE α] {m n : instSI.I} {x y : α} (h : DistLater n x y) (hm : m < n) : x ≡{m}≡ y :=
  h _ hm
...
theorem distLater_succ [OFE α] {n : instSI.I} {x y : α} : DistLater (SIdx.mixin.succ n) x y ↔ x ≡{n}≡ y :=
...
@[rocq_alias discrete]
theorem Discrete.discrete [OFE α] [Discrete α] {n : instSI.I} {x y : α} (h : x ≡{n}≡ y) : x = y :=
...
/-- For discrete OFEs, `n`-equivalence implies equivalence for any `n`. -/
theorem Discrete.discrete_n [OFE α] [Discrete α] {n : instSI.I} {x y : α} (h : x ≡{0}≡ y) : x ≡{n}≡ y :=
  (discrete h).dist
...
theorem dist_mk {hlt : ∀ {n m : SI} {x y : X}, dist n x y → m < n → dist m x y}
    {n : instSI.I} {x y : X} : (mkQuotient dist heqv hlt).Dist n (mk x) (mk y) ↔ dist n x y := Iff.rfl
...
instance [OFE α] : OFE SI (ULift α) where
...
instance [OFE α] : OFE SI (Option α) where
...
theorem some_dist_some [OFE α] {n : instSI.I} {x y : α} : (some x ≡{n}≡ some y) ↔ x ≡{n}≡ y := .rfl
@[simp] theorem not_some_dist_none [OFE α] {n : instSI.I} {x : α} : ¬some x ≡{n}≡ none := id
@[simp] theorem not_none_dist_some [OFE α] {n : instSI.I} {x : α} : ¬none ≡{n}≡ some x := id
...
instance [OFE α] [OFE β] : OFE SI (α -n> β) where
...
instance [OFE α] [OFE β] : OFE SI (α -c> β) where
...
instance [OFE α] [OFE β] : OFE SI (α × β) where
...
theorem dist_fst {n : instSI.I} [OFE α] [OFE β] {x y : α × β} (h : x ≡{n}≡ y) : x.fst ≡{n}≡ y.fst := h.left
theorem dist_snd {n : instSI.I} [OFE α] [OFE β] {x y : α × β} (h : x ≡{n}≡ y) : x.snd ≡{n}≡ y.snd := h.right

@[rocq_alias pair_dist]
theorem dist_prod_ext {n : instSI.I} [OFE α] [OFE β] {x₁ x₂ : α} {y₁ y₂ : β}
...
instance [OFE α] (P : α → Prop) : OFE SI (Subtype P) where
...
instance Sigma.fst_ne {P : α → Type _} [OFE α] [∀ x, OFE SI (P x)] :
...
theorem Sigma.dist_snd {P : α → Type _} [∀ x, OFE SI (P x)] {n : instSI.I} {x y : Sigma P}
...
theorem Sigma.mk_dist {P : α → Type _} [∀ x, OFE SI (P x)] {n : instSI.I} {i1 i2 : α} {v1 : P i1} {v2 : P i2}
...
@[simp] theorem Iso.hom_inv_dist [OFE α] [OFE β] (iso : Iso α β) {n : instSI.I} {x} :
    iso.hom (iso.inv x) ≡{n}≡ x :=
  (Iso.hom_inv iso).dist

@[simp] theorem Iso.inv_hom_dist [OFE α] [OFE β] (iso : Iso α β) {n : instSI.I} {x} :
...
theorem Iso.hom_dist [OFE α] [OFE β] (iso : Iso α β) {n : instSI.I} ⦃x y⦄ :
...
theorem Iso.inv_dist [OFE α] [OFE β] (iso : Iso α β) {n : instSI.I} ⦃x y⦄ :
...
`n`-equivalence. -/
@[rocq_alias chain] structure Chain {SI} (α : Type _) [SIdx SI] [OFE α] where
  chain : SI → α
...
@[simp] theorem const_apply [OFE α] {a : α} {n : instSI.I} : const a n = a := rfl
...
@[simp] theorem map_apply [OFE α] [OFE β] {f : α -n> β} {c : Chain α} {n : instSI.I} :
    map f c n = f (c n) := rfl
...
def map (f : α -n> β) {n : instSI.I} (c : BChain α n) : BChain β n where
...
def le {n : instSI.I} (c : BChain α n) {m : SI} (hm : m ≤ n) : BChain α m where
  bchain m' hm' := c.bchain m' (SIdx.lt_le_trans hm' hm)
  bcauchy _ _ h := c.bcauchy _ _ h

@[simp] theorem map_apply {f : α -n> β} {n : instSI.I} {c : BChain α n} {m} {hm : m < n} :
...
@[rocq_alias Cofe]
class IsCOFE (SI : outParam <| Type _) (α : Type _) [SIdx SI] [OFE α] where
  compl : Chain α → α
  conv_compl {c : Chain α} : compl c ≡{n}≡ c n
  lbcompl {n : instSI.I} : SIdx.Limit n → BChain α n → α
  conv_lbcompl {n : instSI.I} (Hn : SIdx.Limit n) (c : BChain α n) {m} (hm : m < n) :
    lbcompl Hn c ≡{m}≡ c.bchain m hm
  lbcompl_ne {n : instSI.I} (hn : SIdx.Limit n) (c1 c2 : BChain α n) {m : SI} :
...
instance instIsCOFEHom [OFE α] [OFE β] [IsCOFE SI β] : IsCOFE SI (α -n> β) where
  compl c := by
...
instance instIsCOFEProd [OFE α] [OFE β] [IsCOFE SI α] [IsCOFE SI β] : IsCOFE SI (α × β) where
...
@[rocq_alias sum_cofe]
instance instIsCOFESum  [OFE α] [OFE β] [IsCOFE SI α] [IsCOFE SI β] : IsCOFE SI (α ⊕ β) where
...
theorem Sigma.bchain_const_proj1 {P : α → Type _} [∀ x, OFE SI (P x)]
    {n : instSI.I} (hn : SIdx.Limit n) (c : BChain (Sigma P) n) {m} (hm : m < n) :
...
def Sigma.bchain_map_snd {P : α → Type _} [∀ x, OFE SI (P x)]
    {n : instSI.I} (hn : SIdx.Limit n) (c : BChain (Sigma P) n) :
...
theorem Sigma.lbcompl_cast {P : α → Type _} [∀ x, OFE SI (P x)] [∀ x, IsCOFE SI (P x)]
    {a b : α} (eq : a = b) {n : instSI.I} (hn : SIdx.Limit n) (c : BChain (P a) n) :
...
theorem Sigma.bchain_cast_apply {P : α → Type _} [∀ x, OFE SI (P x)] {a b : α} (eq : a = b)
    {n : instSI.I} (c : BChain (P a) n) {p} (hp : p < n) :
...
theorem DiscreteO.dist_inj [SIdx SI] {α : Type _} {x y : α} {n : instSI.I} :
...
def optionBChain {n : instSI.I} (c : BChain (Option α) n) (x : α) : BChain α n := by
...
theorem Option.map_ne [OFE β] {f g : α → β} {x y : Option α} {n : instSI.I} :
...
theorem Option.join_ne {x y : Option (Option α)} {n : instSI.I} (hxy : x ≡{n}≡ y) : x.join ≡{n}≡ y.join := by
  cases x <;> cases y <;> simp_all [Dist, Option.Forall₂]

@[rocq_alias from_option_ne]
theorem Option.elim_ne {β : Type _} (R : β → β → Prop) {f g : α → β} {d d' : β}
    {x y : Option α} {n : instSI.I} (hf : ∀ x y, x ≡{n}≡ y → R (f x) (g y)) (hd : R d d')
...
instance instOFunctorContractiveHomOF [OFunctorContractive SI F1] [OFunctorContractive SI F2] :
  ...
  map_contractive.1 {n : instSI.I} ab ab' h := match ab, ab' with
    | ⟨a, b⟩, ⟨a', b'⟩ => by
...
class LimitPreserving [COFE SI α] (P : α → Prop) : Prop where
  ...
  lbcompl {n : instSI.I} (hn : SIdx.Limit n) (c : BChain α n) :
...
theorem conv_bcompl {n : instSI.I} (c : BChain α n) {m} (hm : m < n) :
...
theorem bcompl_ne {n : instSI.I} (c1 c2 : BChain α n) {m : SI}
...
instance isOFE_later [OFE A] : OFE SI (Later A) where
...
def laterLimitBChain [OFE A] {n : instSI.I} (c : BChain (Later A) n) (hn : SIdx.Limit n) :
    BChain A n where
...
instance isCOFE_later [OFE A] [IsCOFE SI A] : IsCOFE SI (Later A) where
